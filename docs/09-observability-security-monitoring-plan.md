...
### 2.2 実装証跡（コード）
1. `/metrics` endpoint: `apps/idp-server/src/app.ts`
2. Prometheus metrics定義: `apps/idp-server/src/core/metrics.ts`
3. HTTP latency/requests計測: `apps/idp-server/src/middleware/http-metrics.ts`
4. `/metrics` のテスト: `apps/idp-server/src/app.test.ts`
5. synthetic check実行スクリプト: `apps/idp-server/src/scripts/synthetic-check.ts`

## 3. 完了定義（Definition of Done）
- [x] 認証系SLI（成功率/レイテンシ/依存先エラー）を収集できる
- [x] `/metrics` でPrometheus scrape可能
- [x] 主要security eventのアラートルールが定義される
- [x] アラート→runbook対応表がドキュメント化される
- [x] 最低2種類のダッシュボード（Reliability/Security）が定義される
- [x] 合成監視（discovery, jwks, login）を導入する
- [x] `pnpm verify` が通る
...
## 12. 実装チェックリスト
- [x] `/metrics` endpoint 実装
- [x] auth/security SLI メトリクス実装
- [x] dependency health metric 実装
- [x] alert rule 文書化
- [x] runbook連携表の整備
- [x] reliability/security dashboard 定義
- [x] synthetic checks 導入
- [x] `pnpm verify` 通過

## 13. 実装完了サマリ（2026-04-26）
- `prom-client` を導入し、`/metrics` endpointを追加した。
- HTTP request count/latency、login success/failure、security event、dependency health/error の指標を実装した。
- OIDC依存先失敗時の `idp_dependency_up` / error counter 連動を追加した。
- alert rules / dashboard定義 / synthetic monitoring手順を `docs/` に追加した。
- `pnpm --filter @idp/idp-server synthetic:check` で discovery/jwks/login を定期実行可能にした。
- `pnpm verify` 通過を確認した。
